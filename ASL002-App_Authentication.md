...
* Creation of OTU tokens will be limited to a maximum of 3 **live** (i.e.
    issued and not yet consumed) tokens per `client-id` plus `username`
    combination. Issuance of a 4th live token for a given pair will cause
...
* Limits on token creation:
    ...
    * Creation of OTU tokens will be limited per `client-id` to 1 per second.
    ...
    exhaustion for 20 seconds will result in a 5 minute (or longer) `client-id`-level
    block.
...
* Limits on token validation:
    ...
    * Authentication validation requests will be limited to a maximum of 10
    ...
    for 20 seconds will result in a 5 minute (or longer) IP-level block.
